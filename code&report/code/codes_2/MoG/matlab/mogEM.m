...
function [p,mu,vary,logProbX] = mogEM(x,K,iters,minVary,plotFlag,Kmeans)
...
%------------------------------------------------------------------------
vary = vr*ones(1,K)*2;  % diagonal elements of ¦²
...
for i=1:iters
  % Do the E step
  ivary = 1./vary;  % diagonal elements of ¦²^(-1)
  logNorm = log(p)-0.5*N*log(2*pi)-0.5*sum(log(vary'),2);   % ln(¦Ğk)-1/2ln(2¦Ğ|¦²|)
  logPcAndx = zeros(K,T);
  for k=1:K     % log(¦Ğk*N(x)), K¡ÁT
    logPcAndx(k,:) = logNorm(k)...
                - 0.5*sum((ivary(:,k)*ones(1,T)).*(x-mu(:,k)*ones(1,T)).^2,1);
  end
  [mx, mxi] = max(logPcAndx,[],1);  % mxi is the class with max probability mx
  PcAndx = exp(logPcAndx-ones(K,1)*mx); % subtract max to avoid overflow when evaluating exp
  Px = sum(PcAndx,1);               % denominator of ¦Ã(z_k), 1¡ÁT
  PcGivenx = PcAndx./(ones(K,1)*Px);% poterior ¦Ã(z_k), K¡ÁT
  logProb = log(Px) + mx;           % add back max
  logProbX(i) = sum(logProb);       % log-likelihood

  % Plot log prob of data
  figure(1);
  set(gcf,'DoubleBuffer','on')  % gcff: current figure handle
  clf;
  ...
  if plotFlag     % Plot the data and Gaussians
    ...
    clf;
    hold on;
    for k=1:K   % Only plot the first two dimension to form a ellipse (cov=0)
  ...
  respTot = mean(PcGivenx,2);   % ¦Ğ_k = N_k/N=1/N¦²¦Ã(z_k), K¡Á1
  respX = zeros(N,K);           % 1/N¦²¦Ã(z_k)x
  respDist = zeros(N,K);        % 1/N¦²¦Ã(z_k)(x-¦Ì)(x-¦Ì)'
  for k=1:K
  ...
  % Do the M step
  p = respTot;  % update ¦Ğ_k
  mu = respX./(ones(N,1)*respTot'+eps); % update ¦Ì_k (1/N_k = 1/N/¦Ğ_k)
  vary = respDist./(ones(N,1)*respTot'+eps);     % update ¦²_k
  vary = (vary>=minVary).*vary + (vary<minVary)*minVary;
